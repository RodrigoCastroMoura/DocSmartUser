    ...
    try:
        ...
        for page_num in range(len(pdf_reader.pages)):
            ...
            for sig in signatures_by_page[page_num]:
                ...
                # Criar objeto de imagem com tratamento de transparência
                img_buffer = io.BytesIO(sig_img_bytes)
                pil_image = Image.open(img_buffer).convert('RGBA')
                img = ImageReader(pil_image)

                # Desenhar a assinatura no canvas com configurações otimizadas
                can.drawImage(
                    img, 
                    float(sig['x']), 
                    float(sig['y']), 
                ...
                # Garantir que o canvas seja atualizado
                can.showPage()
